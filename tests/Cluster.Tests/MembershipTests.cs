...
public class MembersStoreTests
{
    ...
    public async Task GossipRefreshesAnUnverifiedAddressAndLeavesAProvenOneAlone()
    {
        ...
        await store.UpdateMembershipAsync(
            unproven.Id, GossipState.Alive, 1, now, [new MemberCandidate("http://10.0.0.9:8080", true)], "v1", null, authoritative: true, default);
        ...
        await store.UpdateMembershipAsync(
            proven.Id, GossipState.Alive, 1, now, [new MemberCandidate("http://10.0.0.9:8080", true)], "v1", null, authoritative: true, default);
